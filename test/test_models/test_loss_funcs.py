...
class TestTorchLossWrapper(unittest.TestCase):
    ...
    def test_call(self):
        ...
        loss = self.torch_loss_object(model_output, dataloader_data, self.dummy_trainer)
    ...
    def test_tracked_loss_is_float(self):
        ...
        self.dummy_trainer.mode = ModelTrainerMode.TRAIN
        model_output = torch.tensor([1.0, 2.0, 3.0]).cuda()
        dataloader_data = [torch.tensor([1.0, 2.0, 3.0]).cuda()] * 2
        _ = self.torch_loss_object(model_output, dataloader_data, self.dummy_trainer)
    ...
    def test_loss_tracker_epoch_ended_correct_length(self):
        ...
        _ = self.torch_loss_object(model_output, dataloader_data, self.dummy_trainer)
        _ = self.torch_loss_object(model_output, dataloader_data, self.dummy_trainer)
        self.torch_loss_object.loss_tracker_epoch_ended(2)
        self.assertEqual(len(self.torch_loss_object.loss_tracker.epoch_losses["train_loss"]), 1)
...
class TestBLPathlengthLoss(unittest.TestCase):
    ...
    def test_call(self):
        ...
        loss = self.loss_func(model_output, dataloader_data, self.dummy_trainer)
...
class TestSumLoss(unittest.TestCase):
    ...
    def test_call(self):
        ...
        loss = self.sum_loss(model_output, dataloader_data, self.dummy_trainer)
    ...
    def test_losses_are_tracked_properly(self):
        ...
        model_output = torch.tensor([1.0]).cuda()
        dataloader_data = [torch.tensor([2.0]).cuda()] * 2
        self.sum_loss(model_output, dataloader_data, self.dummy_trainer)
