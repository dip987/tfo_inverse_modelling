...
data1 = interpolate_exp_transform(data, experiment_sdd, [1.0, 0.8])
...
fixed_columns = ['Maternal Wall Thickness', "Maternal Hb Concentration",
                 "Maternal Saturation", "Fetal Hb Concentration"]
# Just to make sure I don't write stupid code
assert (len(fixed_columns) ==
        4), "4/5 TMPs should remain fixed. fixed_column length should be 4"
...
data[y_column] = y_scaler.fit_transform(
    data[y_column].to_numpy().reshape(-1, 1))
...
model_config = {
    ...
    # Input params as an array
    'model_params':  [[IN_FEATURES, 10, 4, OUT_FEATURES]],
    'train_split': 0.8,
    'epochs': 25,
...
train, val = generate_differential_data_loaders(data, params, fixed_columns, x_columns, y_column,
                                                model_config['total_data_len'], model_config["allow_zero_diff"],
                                                model_config['train_split'])
...
train_loss, val_loss = train_model(model, optimizer=optimizer, criterion=criterion,
                                   train_loader=train, validation_loader=val, epochs=4)
